...
ciphertexts = [bytes.fromhex(c) for c in ciphers_hex]
...
def guess_target_from_space(ciphertexts, target):
    ...
    for i, cipher in enumerate(ciphertexts):
        xor_result = xor_bytes(cipher, target)
        for j in range(len(xor_result)):
            char = xor_result[j]
            if 65 <= char <= 90 or 97 <= char <= 122:  
                possible = chr(char ^ 0x20)
                if guess[j] == '.':
                    guess[j] = possible
                elif guess[j] != possible:
                    guess[j] = '?'
    return ''.join(guess)


print("\nXOR entre pares de ciphertexts:")
for i, j in combinations(range(len(ciphertexts)), 2):
    xor_result = xor_bytes(ciphertexts[i], ciphertexts[j])
    output = ''.join([chr(b) if 32 <= b <= 126 else '.' for b in xor_result])
    print(f"C{i+1} ⊕ C{j+1}: {output}")

print("\nXOR del target con otros ciphertexts:")
for i, cipher in enumerate(ciphertexts):
    xor_result = xor_bytes(cipher, target)
    output = ''.join([chr(b) if 32 <= b <= 126 else '.' for b in xor_result])
    print(f"Target ⊕ C{i+1}: {output}")

target_guess = guess_target_from_space(ciphertexts, target)
print("\nPosible contenido del mensaje target:")
print(target_guess)
